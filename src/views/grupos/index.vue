  ...
  methods: {
    async changeStatus1(item) {
      ...
      try {
        ...
        const resp = await GroupService.updateGroup(item.id, updateData, this.$store);
    ...
    async deleteData(id) {
      ...
      const resp = await GroupService.deleteGroup(id, this.$store)
    ...
    async getAllData() {
      ...
      const url =
        ...
        JSON.stringify(this.arrayFilters)
        console.log("HOLA")
      const resp = await GroupService.getGroups(url, this.$store)
