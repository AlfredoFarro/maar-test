      ...
      <b-card ref="filterContent" no-body class="sticky">
        <b-card-body>
          <b-row>
            ...
            <b-col lg="3" class="col-xxl">
              <b-form-group label="Tipo" label-for="type" class="mr-2">
                <v-select
                  :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'"
                  :options="tipos"
                  input-id="type"
          ...
          >
            ...
            <template #cell(actions)="data">
              ...
              <b-button
                  size="sm"
                  class=""
                  @click.prevent="reactivateTravel(data.item)"
  ...
  methods: {
    ...
    async exportDetailCrono(id){
      if (id != null) {
          ...
          // console.log('resp',resp)
          var fileLink = document.createElement('a')
          fileLink.href = this.apiurl + '/travel/detail/download/' + id + url
          console.log('fileLink.href', fileLink.href)
          fileLink.setAttribute('target', '_blank')
          document.body.appendChild(fileLink)
          fileLink.click()
    ...
    async reactivateData(id) {
      console.log('id del cronograma', id)
      const resp = await TravelService.reactivateAllTravel(id, this.$store)
    ...
    changeForms(item){
      ...
      this.$router.push('/registros/aprobar/' + item.id)
 ...
 */      
    ...
    async getAllData() {
      ...
      const url =
        ...
        JSON.stringify(this.arrayFilters)
      console.log('URL', url)
      const resp = await TravelService.getRecords(url, this.$store)
    
      console.log('resp RECORDS', resp)
      if (resp.status) {
        this.allData = resp.data.travels;
        let filteredData
