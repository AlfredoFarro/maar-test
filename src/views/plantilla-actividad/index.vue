...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'type', label: 'Tipo de Actividad', sortable: false, visible: true, thStyle: { width: '160px' } },
        { key: 'userRegister.fullname', label: 'Última modificación por', sortable: false, visible: true, thStyle: { width: '150px' } },        
      ...
      proyectos: [],
      sedes: [],
      servicios: ['Abastecimiento de combustible', 'Mantenimiento de la unidad', 'Prueba en ruta', 'Viaje en vacío'],      usuarios: [],
      arrayFilters: [],
      currentPage: 1,
  ...
  methods: {
    ...
    addPlantilla(id){
      if (id != null) {
          this.$router.push('/plantilla-actividad/registro/' + id)
        } else {
          this.$swal({
            icon: 'warning',
            title: 'Proyecto',
    ...
    async edit(item) {
      ...
      this.$router.push('/plantilla-actividad/editar/' + item.id)
    },
    selectAll(val) {
    ...
    async deleteData(id) {
      ...
      const resp = await TemplateService.deleteTemplate(id, this.$store)
    ...
    filter() {
      ...
      if (this.service != null && this.service != '') {
        this.arrayFilters.push({ keyContains: 'type', key: 'equals', value: this.service })
      }
    ...
    async getAllData() {
      ...
      const url =
        ...
        JSON.stringify(this.arrayFilters)
        console.log("url", url)
      const resp = await TemplateService.getTemplates(url, this.$store)
