    ...
    <filters :filtros="fields" :is-add.sync="isAdd" ref="filters"> </filters>
                ...
                <b-table :items="rowsDespacho" :fields="fields">
                    ...
                    <template v-slot:cell(actions)="row">
                        <b-button
                            size="sm"
                            class=""               
                            @click.prevent="showFlow(1,row.index)"
                        ...
                        <!-- <b-button
                          size="sm"
                          class=""      
                          v-if="row.item.canReactivate"         
                          @click.prevent="reactivate(row.item)"
                ...
                <b-table :items="rowsEmbarque" :fields="fields">
                    ...
                    <template v-slot:cell(actions)="row">
                      <b-button
                            size="sm"
                            class=""               
                            @click.prevent="showFlow(2,row.index)"
                            v-b-tooltip.noninteractive.hover.left="'Ver Flujo'"
                            variant="flat-success"               
                            >
                            <feather-icon size="20" icon="EyeIcon" />
                ...
                <b-table :items="rowsConduccion" :fields="fields">
                    ...
                    <template v-slot:cell(actions)="row">
                      <b-button
                            size="sm"
                            class=""               
                            @click.prevent="showFlow(3,row.index)"
                            v-b-tooltip.noninteractive.hover.left="'Ver Flujo'"
                            variant="flat-success"               
                            >
                            <feather-icon size="20" icon="EyeIcon" />
                ...
                <b-table :items="rowsFinalizacion" :fields="fields">
                    ...
                    <template v-slot:cell(actions)="row">
                      <b-button
                            size="sm"
                            class=""               
                            @click.prevent="showFlow(4,row.index)"
                            v-b-tooltip.noninteractive.hover.left="'Ver Flujo'"
                            variant="flat-success"               
                            >
                            <feather-icon size="20" icon="EyeIcon" />
      ...
      <div v-if="showingContent && isPhoto">
        ...
        <div v-else-if="showingContent && !isPhoto">
          ...
          @click="downloadForm"
          class="mb-2 ml-2">
              <feather-icon style="width: 15px; height: 15px;" icon="DownloadIcon"/> Descargar
          </b-button>
...
export default {
  ...
  data() {
    return {
      ...
      rowsFinalizacion: [],
      fields: [
        { key: 'formName', label: 'Formulario', sortable: false, visible: true, thStyle: { width: '240px' }  },
        { key: 'userLastUpload.fullname', label: 'Subido por', sortable: false, visible: true, thStyle: { width: '240px' }  },
        { key: 'state', label: 'Estado', sortable: false, visible: true, thStyle: { width: '130px' } },
        { key: 'mandatory', label: 'Obligatorio', sortable: false, visible: true, thStyle: { width: '130px' } },
       /*  { key: 'code', label: 'Código', sortable: false, visible: true, thStyle: { width: '155px' } },
        { key: 'dateInit', label: 'Fecha Inicio', sortable: false, visible: true, thStyle: { width: '150px' } },
        { key: 'dateEnd', label: 'Fecha Fin', sortable: false, visible: true, thStyle: { width: '150px' } },        
        { key: 'type', label: 'Tipo', sortable: false, visible: true, thStyle: { width: '100px' } },        
        { key: 'route', label: 'Ruta', sortable: true, visible: true, thStyle: { width: '160px' } },
        { key: 'primaryPilot.fullname', label: 'Piloto', sortable: true, visible: true, thStyle: { width: '160px' } }, */
        { key: 'errorMsg', label: 'Motivo Rechazo', sortable: false, visible: true, thStyle: { width: '300px' } },
        { key: 'actions', label: 'Acciones', visible: true, thStyle: { width: '150px' } }
      ],
      estados: [
        {
          text: 'Oficializado',
  ...
  methods: {
    ...
    filter() {
      ...
      if (this.userData.id != null &&  this.userData.id != '') {
        this.arrayFilters.push({ keyContains: 'userId', key: 'equals', value:  this.userData.id })
      }
    ...
    showFlow(section,index){
      console.log("MOSTRAR FLUJO")
      switch (section) {
        case 1:
          this.props = this.rowsDespacho[index].approvalFlow
          this.hasContent = this.rowsDespacho[index].content ? true : false
          this.allContent = this.rowsDespacho[index].allPhotos
          this.idRow = this.rowsDespacho[index].id
          this.stateRow = this.rowsDespacho[index].state
          break;
        case 2: 
          this.props = this.rowsEmbarque[index].approvalFlow
          this.hasContent = this.rowsEmbarque[index].content ? true : false
          this.allContent = this.rowsEmbarque[index].allPhotos
          this.idRow = this.rowsEmbarque[index].id
          this.stateRow = this.rowsEmbarque[index].state
          break;
        case 3:
          this.props = this.rowsConduccion[index].approvalFlow
          this.hasContent = this.rowsConduccion[index].content ? true : false
          this.allContent = this.rowsConduccion[index].allPhotos
          this.idRow = this.rowsConduccion[index].id
          this.stateRow = this.rowsConduccion[index].state
          break;
        case 4:
          this.props = this.rowsFinalizacion[index].approvalFlow
          this.hasContent = this.rowsFinalizacion[index].content ? true : false
          this.allContent = this.rowsFinalizacion[index].allPhotos
          this.idRow = this.rowsFinalizacion[index].id
          this.stateRow = this.rowsFinalizacion[index].state
          break;
        default:
          break;
      }
    ...
    async reactivate(index){
      console.log("REACTIVAR", index)
      this.$swal({
        title: '¿Está seguro?',
        text: '¿Desea reactivar el formulario?',
        icon: 'warning',
        ...
        if (result.isConfirmed) {
          const resp = await ActivityService.reactivateForm(index.id, this.$store)
          console.log("RESP REACTIVAR", resp)
          if (resp.status) {
            this.$swal({
              title: 'Reactivado',
              text: 'El formulario ha sido reactivado.',
              icon: 'success',
              customClass: {
                confirmButton: 'btn btn-primary'
              },
            ...
            this.$swal({
              title: 'Error!',
              text: 'Hubo un error al reactivar el formulario',
              icon: 'error',
              customClass: {
                confirmButton: 'btn btn-primary'
              },
    ...
    downloadForm(){
      if (this.idRow != null) {
        ...
        fileLink.href =
          this.apiurl + "/record-activity-user/downloadDeclaration/" + this.idRow;
    ...
    async getAllData() {
      ...
      const url =
        ...
        JSON.stringify(this.arrayFilters)
      console.log("url,", url)
      const resp = await ActivityService.getRecordsById(this.$route.params.id, this.$store)
      console.log("resp RECORD FORMS ID", resp)
      this.isFinished = resp.data[0].state == 2 ? true : false

      if (resp.status) {
        this.rowsDespacho = resp.data[0].formsDespacho;
        this.rowsEmbarque = resp.data[0].formsEmbarque;
        this.rowsConduccion = resp.data[0].formsConduccion;
        this.rowsFinalizacion = resp.data[0].formsFinalizacion;
        console.log("rowsDespacho", this.rowsDespacho)
        console.log("rowsEmbarque", this.rowsEmbarque)
        console.log("rowsConduccion", this.rowsConduccion)
        console.log("rowsFinalizacion", this.rowsFinalizacion)
