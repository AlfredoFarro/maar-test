        ...
        <b-card-body>
          <b-row>
            <b-col md="12" lg="9" class="d-flex flex-column flex-lg-row justify-content-start">
              ...
              <div class="w-50">
                <b-form-group label="Tipo de Servicio" label-for="service" class="mr-2">
                  <v-select
                  :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'"
                  :options="servicios"
                  input-id="service"
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'type', label: 'Servicio', sortable: false, visible: true, thStyle: { width: '160px' } },
  ...
  methods: {
    ...
    addPlantilla(id){
      if (id != null) {
          this.$router.push('/plantilla/registro/' + id)
        } else {
          this.$swal({
            icon: 'warning',
            title: 'Proyecto',
    ...
    async edit(item) {
      ...
      this.$router.push('/plantilla/editar/' + item.id)
    },
    selectAll(val) {
