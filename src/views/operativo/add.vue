...
export default {
    ...
    methods: {
        ...
        async saveData(templateExists = true) {
            ...
            resp = await TravelService.saveTravel(this.dataItems, this.$store)
        ...
        async validGeneral() {
            ...
            return new Promise((resolve, reject) => {
                ...
                if (success) {
                    ...
                    this.arrayFilters.push({ keyContains: 'projectId', key: 'equals', value: data.projectId })
                    this.arrayFilters.push({ keyContains: 'type', key: 'equals', value: data.type })
                    this.arrayFilters.push({ keyContains: 'sedeId', key: 'equals', value: data.sedeId })
                    const url =
                    `?limit=1000000&page=1&order=ASC&sort=id&filter=` +
                    JSON.stringify(this.arrayFilters)
                    const resp = await TravelService.getTemplateTravel(url, this.$store)
                    console.log("URL", url)
