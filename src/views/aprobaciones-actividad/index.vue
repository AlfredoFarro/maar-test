  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'route', label: 'Descripción', sortable: true, visible: true, thStyle: { width: '250px' } },
  ...
  methods: {
    async approveAction(item){
      ...
      this.$swal
        ...
        .then(async result => {
          if (result.value) {
            const resp = await ActivityService.approvalForms(
              {
                data: [
                  {
                    id: item.id,
    ...
    filter() {
      ...
      if(this.project_id != null && this.project_id != ''){
        this.arrayFilters.push({ keyContains: 'activity.projectId', key: 'equals', value: this.project_id })
      }
    ...
    async getAllData() {
      ...
      const url =
        `?limit=10000&filter=` +
        JSON.stringify(this.arrayFilters)
      console.log("ARRAY FILTERSs", this.arrayFilters)
      const resp = await ActivityService.getActivityByProfileWeb('', this.$store)
    
      console.log('resp FORMS ACTIVITIES', resp)
      ...
      if (resp.data.length > 0) {
        ...
        /* this.allData = this.allData.filter(obj => {
          ...
          if (obj.currentLvl !== 0 && obj.currentLvl !== 3 && obj.state !== 3) {
            // Verificar si groupLvl1 o groupLvl2 tienen un ID que está en userGroups
            if ((this.userGroups.includes(obj.groupLvl1?.id) && obj.currentLvl !== 2) || 
              (obj.groupLvl2 && this.userGroups.includes(obj.groupLvl2.id) && obj.currentLvl === 2)) {
