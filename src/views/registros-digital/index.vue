<template>
  ...
  <div class="w-100">
    <b-overlay
      ...
      rounded="sm"
    >
      <add-edit :is-add.sync="isAdd" ref="sedeAdd"> </add-edit>
          ...
          <b-row>
            ...
            <b-col md="6" lg="3" class="d-flex">
              <div
                class="d-flex align-items-center h-100 justify-content-center justify-content-lg-start justify-content-xl-center mb-1 mb-lg-0 mt-02"
              >
                <b-button class="mr-2" variant="primary" @click="downloadExcel">
                ...
                <b-button
                  ...
                  class="mr-2"
                  variant="danger"
                  @click="downloadPdfBulk"
                >
                  <span class="text-nowrap">
                    <feather-icon icon="FileIcon" /> Exportar PDF
                  </span>
                </b-button>
              </div>
            </b-col>
...
import VueStoreService from "@/services/VueStoreService";
...
export default {
  ...
  methods: {
    ...
    edit(item) {
      ...
      this.form.disciplina =
        item.record_discipline && item.record_discipline.length > 0
    ...
    filter() {
      ...
      this.arrayFilters.push({
        keyContains: "project.isActive",
        key: "equals",
        value: 1,
      });
      this.arrayFilters2.push({
        keyContains: "isActive",
        key: "equals",
        value: 1,
      });
      ...
      const isShortDni = this.dniFilter && this.dniFilter.trim().length <= 4;

      if (isShortDni) return;

      if (this.selectedWorker && this.selectedWorker.id) {
        this.arrayFilters.push({
          keyContains: "user.id",
          key: "equals",
          value: this.selectedWorker.id,
        });
      } else if (this.dniFilter && this.dniFilter.trim().length > 4) {
        this.arrayFilters.push({
          keyContains: "user.document",
          key: "contains",
          value: this.dniFilter.trim(),
        });
      }
    ...
    onDniInput() {
      if (this.dniFilter && this.dniFilter.trim().length > 0) {
    ...
    async downloadPdfBulk() {
      if (!this.canExportPdf) return;

      this.loading = true;
      try {
        const qs =
          `limit=100000&sort=completed&order=desc&filter=` +
          encodeURIComponent(JSON.stringify(this.arrayFilters || []));

        const file = await VueStoreService.getFile2(
          `record/pdf?${qs}`,
          this.$store,
        );
    ...
    async downloadPdf(item) {
      ...
      try {
        const blob = (await this.$store.dispatch)
          ? await (
              await import("@/services/VueStoreService")
            ).default.getFile2(`record/${item.id}/pdf`, this.$store)
          : null;

        const url = window.URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url;
        a.download = `registro-digital-${item.id}.pdf`;
    ...
    async searchWorkers(search, loading) {
      const q = (search || "").trim();
      ...
      if (this.workerSearchTimer) clearTimeout(this.workerSearchTimer);

      this.workerSearchTimer = setTimeout(async () => {
        try {
          const filters = [];

          if (/^\d+$/.test(q)) {
            filters.push({
              keyContains: "document",
              key: "contains",
              value: q,
            });
          } else {
          ...
          const url =
            `?limit=20&sort=fullname&order=asc&filter=` +
          ...
          if (
            ...
            this.workerOptions = resp.data.rows.map((u) => ({
              ...u,
              label: `${u.document || ""} - ${u.fullname || ""}`.trim(),
            }));
          } else {
            this.workerOptions = [];
          }
        } catch (e) {
          console.log("Error buscando trabajadores:", e);
          this.workerOptions = [];
        } finally {
          loading(false);
        }
