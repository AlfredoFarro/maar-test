  ...
  <div class="w-100">
    ...
    <!-- <filters :filtros="fields" :is-add.sync="isAdd" ref="filters"> </filters> -->
      <b-card ref="filterContent" no-body class="sticky">
        <b-card-body>
          <b-row>
            <b-col md="12" lg="9" class="d-flex flex-column flex-lg-row justify-content-start">
              ...
              <div class="w-100">
                <b-form-group label="Última modificación por" label-for="user" class="mr-2">
                  <v-select
                    :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'"
                    :options="usuarios"
                    label="fullname"
        ...
        <div class="table-overflow">
          ...
          >
            ...
            <template #cell(isActive)="data">
              <span v-if="data.item.isActive" @click="changeState(data.item)" style="padding: .7rem 1.5rem; background-color: #28c76f; border-radius: 50px; color: white">Activo</span>
              <span v-else @click="changeState(data.item)" style="padding: .7rem 1.5rem; background-color: #ea5455; border-radius: 50px; color: white">Inactivo</span>
            </template>
          </b-table>
        </div>
        <div class="mx-2 mb-2">
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: 'project.description', label: 'Proyecto', sortable: false, visible: true, thStyle: { width: '155px' } },
        { key: 'sede.description', label: 'Sede', sortable: false, visible: true, thStyle: { width: '160px' } },
        /* { key: 'userRegister.fullname', label: 'Usuario', sortable: false, visible: true, thStyle: { width: '150px' } },         */
        { key: 'created_at', label: 'Fecha', sortable: true, visible: true, thStyle: { width: '160px' } },
        { key: 'isActive', label: 'Estado', sortable: true, visible: true, thStyle: { width: '80px' } },
      ],
      form: {
          document: '',
          email: '',
  ...
  methods: {
    ...
    addPlantilla(id){
      if (id != null) {
          this.$router.push('/plantilla-operativo/registro/' + id)
        } else {
          this.$swal({
            icon: 'warning',
            title: 'Proyecto',
    ...
    async edit(item) {
      ...
      this.$router.push('/plantilla-operativo/editar/' + item.id)
    },
    selectAll(val) {
    ...
    validDate(fecha) {
      ...
      if (fecha != null) {
        ...
        return moment(fecha, 'yyyy-MM-DD HH:mm').utc(fecha).format('DD-MM-YYYY')
      }
      return ''
    },
    ...
    async changeState(data){
      ...
      this.$swal.fire({
            ...
            if (result.value) {
              //LLAMAR SERVICIO CAMBIAR ESTADO POR ID DE PLANTILLA
              const resp = await OperateService.changeTemplatesOperateState(data.id, this.$store)
              ...
              if (resp.status) {
                ...
                this.$swal({
                  icon: 'success',
                  title: 'Cambiado!',
                  text: 'El estado fue cambiado con éxito.',
                  customClass: {
                    confirmButton: 'btn btn-success'
                  }
                })
                ...
                this.$swal({
                  icon: 'error',
                  title: 'Error',
                  text: 'Ocurrió un error al cambiar el estado de la plantilla seleccionada.',
                  customClass: {
                    confirmButton: 'btn btn-success'
                  }
                })
    ...
    async deleteData(id) {
      console.log('id del template', id)
      const resp = await OperateService.deleteTemplateOperate(id, this.$store)
    ...
    filter() {
      ...
      if (this.user != null && this.user != '') {
        this.arrayFilters.push({ keyContains: 'userRegister.id', key: 'equals', value: this.user })
      }
    ...
    async getAllData() {
      ...
      const url =
        ...
        JSON.stringify(this.arrayFilters)
        console.log("url", url)
      const resp = await OperateService.getTemplatesOperate(url, this.$store)
    
      console.log('resp TEMPLATE', resp)
      if (resp.status) {
        this.allData = resp.data.rows;
        console.log("ALL DATA",this.allData)

        console.log("AFTER ALL DATA",this.allData)
    ...
    async changeStatus() {
      ...
      this.$swal({
        ...
        if (result.value) {
          const resp = await OperateService.changeTemplatesOperateState(this.id, this.$store)
